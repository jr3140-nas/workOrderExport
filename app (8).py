...
def _create_detail_table_figures(
    ...
    # Wrap header labels to avoid text overflowing.  Insert newline
    # characters at spaces so that long header names (e.g., "Work Order #")
    # span multiple lines within their allocated column width.  We avoid
    # breaking long words or hyphenating.
    header_wrapped: List[str] = []
    for lbl in col_labels:
        # Use ``textwrap.fill`` to wrap at spaces; width of 12 chars chosen
        # empirically to fit within the column widths defined above.  The
        # ``break_long_words`` and ``break_on_hyphens`` flags are disabled
        # to prevent splitting words and maintain readability.
