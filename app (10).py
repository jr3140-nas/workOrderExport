...
def _df_for_pdf(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = df.copy()
    # Remove cost center related columns for the PDF.  The PDF tables are
    # designed for six columns (Name, Work Order #, Sum of Hours, Type,
    # Description and Problem) and do not include cost center fields.  If
    # these columns are present in the input, drop them to avoid a
    # mismatch between the number of columns and the predefined column
    # width specifications in `_create_detail_table_figures`.
    for col in ["CostCenter", "Cost Center", "CC"]:
...
def _create_detail_table_figures(
    ...
    # Fixed relative widths for each of the six columns.  These values sum
    ...
    #   Name          : 0.18 (18%)  – a fixed width for the name column
    #   Work Order #  : 0.06 (6%)   – as before
    #   Sum of Hours  : 0.05 (5%)   – as before
    #   Type          : 0.11 (11%)  – slightly increased
    #   Description   : 0.20 (20%) – reduced to make room for the problem column
    #   Problem       : 0.40 (40%) – increased to accommodate longer text
    col_widths = [0.18, 0.06, 0.05, 0.11, 0.20, 0.40]

    figures: List[plt.Figure] = []
    # Convert to a simple DataFrame for PDF output
    df_pdf = _df_for_pdf(df_detail)
    col_labels = df_pdf.columns.tolist()
    # Wrap header labels.  To improve readability and prevent
    # truncation, manually insert line breaks for certain headers.
    # "Work Order #" becomes "Work Order\n#", and "Sum of Hours" becomes
    # "Sum of\nHours".  Other labels are left as-is; long words without
    # spaces are not split, as their columns are sufficiently wide.
    ...
    for lbl in col_labels:
        if lbl == "Work Order #":
            ...
            # Use textwrap.fill to break at spaces if any, but do not break
            # long words or hyphenate.  A small width (10) is used so
            # multi-word headers like "Cost Center" would wrap appropriately
    ...
    # Identify the index of the "Type" column so we can color it later.  If
    # the Type column is missing, ``type_col_index`` will remain ``None``.
    type_col_index = None
    try:
    ...
    for page_num, start in enumerate(range(0, total_rows, rows_per_page), start=1):
        ...
        # Use a narrower axis and center it horizontally to prevent the table
        # from running off the right edge of the page.  A 5% margin is
        # applied to both the left and right (width 0.90, starting at 0.05).
        ...
        # margins).  Each data row is allotted the same base height.  When
        # text wraps within a row, we double the height of that specific row.
        base_height = 0.85 / (rows_per_page + 1)
        # Adjust header row height based on wrapped header labels.  The
        # number of lines in the header is computed by counting newline
        # characters in each wrapped label.  The tallest header cell
        # determines the header height multiplier.
        ...
        for i in range(len(chunk)):
            ...
            # Apply color coding to the Type column based on the value.  The
            # colors are defined in the `_TYPE_COLORS` dictionary used by
            # the dashboard.  If the Type column is present, color the
            # background of that cell accordingly.
            if type_col_index is not None:
                try:
                    type_value = str(chunk.iloc[i]["Type"])
                except Exception:
                    type_value = ""
                color = _TYPE_COLORS.get(type_value, "#FFFFFF")
                # Set the background color of the Type cell
                table[(i + 1, type_col_index)].set_facecolor(color)
        figures.append(fig)
    return figures
...
pdf_bytes = build_pdf(report, selected_label)
