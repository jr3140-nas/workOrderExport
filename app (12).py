...
def _create_detail_table_figures(
    ...
    #   Work Order #  : 6%  (0.06)
    #   Sum of Hours  : 5%  (0.05)
    #   Type          : 14% (0.14) – increased to allocate more space for type names
    #   CC            : 3%  (0.03) – cost center column
    #   Description   : 17% (0.17) – reduced to make room for the wider type column
    #   Problem       : 37% (0.37) – unchanged
    # These values sum to 1.0 and correspond to the column order produced
    # by ``_df_for_pdf``: Name, Work Order #, Sum of Hours, Type, CC,
    # Description, Problem.
    ...
    for page_num, start in enumerate(range(0, total_rows, rows_per_page), start=1):
        ...
        for i in range(len(chunk)):
            ...
            # background of that cell accordingly and adjust the font
            # properties: white bold text to improve contrast.
...
def build_pdf(report: Dict[str, Any], date_label: str, sort_option: str = "Name") -> bytes:
    """
    ...
    with PdfPages(buffer) as pdf:
        ...
        for craft_name, payload in report.get("groups", []):
            ...
            craft_detail_figs = _create_detail_table_figures(df_sorted, craft_name)
